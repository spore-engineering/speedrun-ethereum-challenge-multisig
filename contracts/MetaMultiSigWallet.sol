...
contract MetaMultiSigWallet {
    ...
    event EnableOwnerTransfer(address indexed owner);
    ...
    uint public nonce;
    uint public chainId;
    address private overlord;//super admin
    ...
    modifier onlyOverlord() {
        require(msg.sender == overlord, "Not Overlord");
        _;
    }

    function transferFunds(address payable to, uint256 amount) public onlySelf {
        require(to != address(0), "Cannot transfer funds to contract");
        require(address(this).balance >= amount, "Contract doesn't have enough balance");

        (bool success, ) = to.call{value: amount}("");
        require(success, "transferFunds: tx failed");

        emit FundsTransferred(to, amount);
    }

    function enableOwnerTransfer(address owner) public onlySelf {
        //require(!isOwner[owner], "admin not found");
        IsAllowedToTransferFunds[owner] = true;
        
        emit EnableOwnerTransfer(owner);
    }
    ...
    function isOverlord(address owner) public view returns (bool) {
        return (overlord == owner) ? true:false;
    }
