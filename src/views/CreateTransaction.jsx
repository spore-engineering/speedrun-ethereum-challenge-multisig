      ...
      try {
       if(methodName == "transferFunds"){
         ...
         console.log("🔥🔥🔥🔥🔥🔥",amount)
           ...
           setData(calldata);
           console.log("CALLLLLL", calldata);
       }
       if(methodName == "enableOwnerTransfer"){
        console.log("enableOwnerTransfer selected")
          const calldata = readContracts[contractName].interface.encodeFunctionData("enableOwnerTransfer",[to])
          setData(calldata);
          console.log("CALLLLLL", calldata);
      }
       ...
       setCreateTxnEnabled(true);
        ...
        else if (decodedDataObject.signature === "enableOwnerTransfer(address)"){
          setMethodName("enableOwnerTransfer")
          setSelectDisabled(true)
        }
      ...
      <div style={{ border: "1px solid #cccccc", padding: 16, width: 400, margin: "auto", marginTop: 64 }}>
        <div style={{ margin: 8 }}>
          ...
          <Select value={methodName} disabled={selectDisabled} style={{ width: "100%" }} onChange={ setMethodName }>
            ...
            <Option disabled={true} key="enableOwnerTransfer">enableOwnerTransfer()</Option>
          ...
          <Button
            ...
            onClick={async () => {
              ...
              const isIsAllowedToTransferFunds = await readContracts[contractName].IsAllowedToTransferFunds(recover);
              console.log("isIsAllowedToTransferFunds", isIsAllowedToTransferFunds);
              
              const isOverlord = await readContracts[contractName].isOverlord(recover);
              console.log("isIsAllowedToTransferFunds", isIsAllowedToTransferFunds);
              

              if (isOwner) {
                if(methodName == "transferFunds" && !isIsAllowedToTransferFunds){
                  console.log("ERROR, NOT ALLOWED TO TRANSFER FUNDS.");
                  setResult("ERROR, NOT ALLOWED TO TRANSFER FUNDS.");
                }
                else if(methodName == "enableOwnerTransfer" && !isOverlord) {
                  console.log("ERROR, ONLY OVERLORD CAN GIVE ACCESS TO TRANSFER FUNDS.");
                  setResult("ERROR, ONLY OVERLORD CAN GIVE ACCESS TO TRANSFER FUNDS.");
                }
                else {
